...
orders.each do |order|
    ...
    if order['state']!= 4
      ...
      opay_completed={0=>'尚未',1=>'已经'}[order['isOnlinePaymentCompleted']]
      content += "异常警告>>> 状态#{order_state.nil? ? '未知' : order_state} #{pay_method}支付 #{pay_online}网付 #{opay_completed}完成 方式#{delivery_type}\n"
    end
    index +=1 
    puts content
end
